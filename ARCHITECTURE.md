...
GitHub PagesはWebアプリ本体ではなく、JSONデータの配信場所として使用する。
...
5. JSONは管理・配信用、SQLiteはアプリ実行時用と役割を分ける。
...
ダウンロード途中、JSON不正、ハッシュ不一致、件数不一致、SQLite取込失敗のいずれかが起きた場合は更新を確定せず、既存データを使い続ける。

互換性のない `schemaVersion` を受信した場合も更新せず、アプリ更新が必要であることを診断可能な状態にする。

## 10. 初回起動とオフライン

アプリに初期manifestと初期データを同梱する。

- ローカルDBがなければ、同梱データからDBを構築する。
- ローカルDBがあれば、同梱データよりローカルDBを優先する。
...
- 最低対応はiOS 17 / macOS 14とする。
- 正式名称は「発想の種」、内部名は `IdeaSeed` とする。
- 初期同梱データは各カテゴリ10件とし、運用データ追加時に増やす。
- SQLiteアクセスはApple標準環境のSQLite C APIを利用し、外部依存を増やさない。
