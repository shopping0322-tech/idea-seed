...
- 更新日: 2026-07-19
- 対象: iPhone優先
- 実装状況: PWA公開・シーン生成とログライン生成の主要機能検証完了
- 正式アプリ名: 発想の種
- 内部プロジェクト名: IdeaSeed
...
履歴は検索、1件削除、全削除に対応する。削除操作は誤操作を避けるため、実行前に確認モーダルを表示する。
...
- 生成結果は4枚のカードを短い時間差で表示し、抽選の手触りを作る。
- 初期画面ではシーンの4カードとログラインの6材料を図形で識別できるモードランチャーを表示する。
- 選択後は大見出し、説明、生成ボタンの文言を現在のモードに合わせ、履歴画面でも選択中モードを判別できるようにする。
- メニューと生成画面の往復は短いスライド・フェードでつなぎ、データ読込はアニメーションと並行して行う。
